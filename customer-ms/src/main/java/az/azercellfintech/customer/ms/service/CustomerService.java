...
public class CustomerService {
        ...
        try {
            if (customer.isEmpty())
                sendRejectMessage(
                        ...
                        queueProperties.getTopUp()
                );

            checkBalance(customer.get().getBalance(), topUpTransactionDto.amount());
            customerRepository.save(subtractBalance(customer.get(), topUpTransactionDto.amount()));
        } catch (Exception ex) {
            if (customer.isEmpty())
                sendRejectMessage(
                        topUpTransactionDto.transactionId(),
        ...
        try {
            if (customer.isEmpty())
                sendRejectMessage(
                        ...
                        USER_NOT_FOUND_ERROR.getMessage(),
                        queueProperties.getRefund()
                );

            customerRepository.save(refundBalance(customer.get(), refundTransactionDto.amount()));
        } catch (Exception ex) {
            if (customer.isEmpty())
                sendRejectMessage(
                        refundTransactionDto.transactionId(),
    ...
    private void sendRejectMessage(String transactionId, String USER_NOT_FOUND_ERROR, String queueName) {
        rabbitMessageProducer.sendMessage(
                new RejectTransactionDto(transactionId, USER_NOT_FOUND_ERROR),
                queueName
        );
    }

    private CustomerEntity subtractBalance(CustomerEntity customer, BigDecimal amount) {
        customer.setBalance(customer.getBalance().min(amount));
        return customer;
    }

    private CustomerEntity refundBalance(CustomerEntity customer, BigDecimal amount) {
